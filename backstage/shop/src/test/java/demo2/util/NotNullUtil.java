...
public class NotNullUtil {
	...
	public static String isBlankAlert(Object src) {
		...
		for (Field f : fields) {
			try {
				// 判断属性是否有NotNull注解
				if (f.isAnnotationPresent(com.example.demo2.util.NotNull.class)) {
					// 打开权限
					...
					if (obj instanceof String) {
						...
						if (str == null || "".equals(str.trim())) {
							com.example.demo2.util.NotNull notNull = f.getAnnotation(com.example.demo2.util.NotNull.class);
							return "请填写" + notNull.fieldName() + "！";
						}
					} else {
						if (obj == null) {
							com.example.demo2.util.NotNull notNull = f.getAnnotation(NotNull.class);
							return "请填写" + notNull.fieldName() + "！";
						}
					}
				}
